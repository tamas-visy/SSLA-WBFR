...
def make_ci_bootsrapper(estimator):
    ...
    def bootstrapper(pred, labels, n_samples=100):
        ...
        for _ in range(n_samples):  # TODO how does it not return an array of same values? JM
            sample = np.random.choice(inds,
                                      len(pred))  # sample from the (0,...,len(pred)) vector without replacement for len(pred) times
            try:
                results.append(
                    estimator(pred[sample], labels[sample]))  # get AUC estimates for the shuffled logits vector
            except ValueError:
                continue
...
def classification_eval(preds, labels, threshold=None, prefix=None,
    ...
    **Arguments** 
    ...
    if bootstrap_cis:
        results["roc_auc"], (results["roc_auc_ci_low"], results["roc_auc_ci_high"]) = roc_auc(preds, labels,
                                                                                              get_ci=True)

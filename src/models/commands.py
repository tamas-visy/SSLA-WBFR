...
universal_options = [
    ...
    click.Option(('--limit_train_frac',), default=None, type=float,
                 help="Truncate the training data so <limit_train_frac>"),
    ...
    def __init__(self, *args, **kwargs):
        ...
        cnn_transformer_params = [
            click.Option(("--model_config",), type=str,
                         help="Path to config yaml for model. Params in this file override other command line args and defaults",
                         callback=validate_yaml_or_json),
            click.Option(("--reset_cls_params",), is_flag=True,
                         help="Reset the parameters in a the classifcation layer after loading pretrained model"),
            click.Option(("--freeze_encoder",), is_flag=True, help="Freeze the encoder during training"),
            click.Option(("--resume_model_from_ckpt",), type=click.Path(exists=True),
                         help="Load a model and continue training from the checkpoint"),
            click.Option(("--use_huggingface",), is_flag=True,
                         help="Run the job with huggingface rather than pytorch lightning"),
            click.Option(("--train_mix_positives_back_in",), is_flag=True,
                         help="Keep mixing positive examples back into the batches"),
            ...
            click.Option(("--multitask_daily_features",), is_flag=True,
                         help="Add a loss term for regressing daily features"),
        ]
...
@click.option("--train_participant_dates", type=click.Path(dir_okay=False), callback=read_participant_dates,
              default=None)
@click.option("--eval_participant_dates", type=click.Path(dir_okay=False), callback=read_participant_dates,
              default=None)
@click.option("--test_participant_dates", type=click.Path(dir_okay=False), callback=read_participant_dates,
              default=None)

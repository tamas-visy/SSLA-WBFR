...
========================
`Project repository available here  <https://github.com/behavioral-data/SeattleFluStudy>`
...
def spark_timezone(timezone: str):
    ...
    lifetime while preserving original timezone. This is especially
...
def main(input_path, output_path, max_missing_days_in_window,
    ...
    with materialize_dataset(spark, output_path, schema, rowgroup_size_mb):
        ...
        aggs = [f.sort_array(f.collect_list(f.struct("timestamp", colName))).alias(f"collect_list({colName})") for
                colName in feature_columns] \

...
class FluPosWeakLabler(object):
    def __init__(self, survey_responses, data_location=None, window_size=7, normalize=True, window_onset_min=0,
                 window_onset_max=0):
        """ survey responses """
        ...
        self.results = self.results.groupby(["participant_id", "_date", "first_report_yn"])[
            "is_pos"].any().reset_index()
        self.results["flu_prob"] = 0.0
        self.results.loc[self.results["is_pos"] == True, "flu_prob"] = 1.0

        # import pdb; pdb.set_trace()
        # merges flu results to survey responses
        self.merged_table = self.survey_responses.merge(self.results, on=["participant_id", "_date", "first_report_yn"],
                                                        how="left")
...
class CleanAnnotationLabler(object):
    def __init__(self, survey_responses, window_onset_min=0,
        ...
        dd = self.merged_table.groupby("response_groups")["is_pos"].apply(
            lambda x: x.any() if not x.isnull().all() else np.nan).to_dict()
        ...
        self.merged_table["is_missing"] = ~(
                ((self.merged_table["have_flu"] == 1) & ~pd.isna(self.merged_table["is_pos"])) | (
                self.merged_table["have_flu"] == 0))
...
class DailyFeaturesLabler(object):
    def __init__(self, window_size=7,
        ...
        self.default_features = np.array(
            ([0] * n_non_missing + [1] * (len(feature_cols) - n_non_missing)) * window_size).astype(np.float32)
    ...
    def __init__(self, feather_path,
        ...
        * Each unique event has 43 days
        ...
        self.results = self.results[
            self.results["days_since_onset_v43"].isin(list(range(window_onset_min, window_onset_max + 1)))]
...
class CovidSignalsLabler(object):
    def __init__(self, window_onset_min: int = 0,
        ...
        self.lab_results["date_received"] = pd.to_datetime(self.lab_results["samples_received"], utc=True).astype(
            '<M8[ns]')

        self.weekly_surveys = pd.read_csv("data/processed/COVID_Signals/weekly_surveys.csv",
                                          dtype={"id_participant_external": "str"})
        self.weekly_surveys["date_completed"] = pd.to_datetime(
            self.weekly_surveys["date_of_test_kit_completion"].str.split("|").str[-1], errors="coerce")
        ...
        self.positive_results = self.results_and_surveys[self.results_and_surveys["result"] == "Positive"].dropna(
            subset="date_completed")
        mapper = lambda x: get_dates_around(x, days_minus=window_onset_min, days_plus=window_onset_max)
...
class ConstantLabler:
    def __call__(self, participant_id=None, start=None, end=None, *, embedded_row1=None, embedded_row2=None):
        return -1
...
class SameParticipantLabler(object):
    ...
    def __call__(self, participant_id=None, start=None, end=None, *, embedded_row1=None, embedded_row2=None):
        if embedded_row1 is not None and embedded_row2 is not None:
            return int(embedded_row1["participant_id"] == embedded_row2["participant_id"])
        return -1
